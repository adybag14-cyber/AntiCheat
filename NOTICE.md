# Publication and licensing notice

This repository is an independent static-analysis research project. It is not
affiliated with, endorsed by, or sponsored by Activision, Microsoft, or the
RICOCHET Anti-Cheat team.

The repository's MIT license applies to repository-authored scripts, tests, and
original explanatory text. It does not grant rights to third-party software,
trademarks, binaries, embedded resources, or decompiled source.

The following material is intentionally excluded:

- `Randgrid.sys` and all other game/application executables;
- extracted .NET single-file bundles and application assemblies;
- Ghidra project databases and bulk decompiler output;
- live named-pipe, device, service, game, or network captures;
- private identifiers, credentials, and machine-specific operational data.

Published evidence is limited to facts and compact derived metadata such as
cryptographic hashes, PE section/import information, API names, selected static
instruction addresses, counts, and original analysis conclusions. Users must
obtain any analyzed software lawfully and comply with its license and applicable
law.

The research is defensive and educational. It does not include or invite
anti-cheat bypass, disabling, evasion, cheating automation, or concealment work.
